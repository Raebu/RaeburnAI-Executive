...
class BriefingEngine:
    ...
    def _summary(self, kpis, risks, sales, emails) -> str:
        high_count = len(
            [
                item
                for item in [*risks, *sales, *emails]
                if item.priority in {Priority.high, Priority.urgent}
            ]
        )
        attention_kpis = [
            k.name for k in kpis if k.status in {Priority.high, Priority.urgent}
        ]
        kpi_text = (
            ", ".join(attention_kpis) if attention_kpis else "no major KPI exceptions"
        )
        return (
            f"Today's briefing highlights {high_count} high-priority signals. "
            f"Watch {kpi_text}. Focus on retention, board readiness and late-stage "
            "sales execution."
        )
    ...
    def _actions(self, risks, sales, calendar) -> list[SuggestedAction]:
        ...
        return [
            SuggestedAction(
                ...
                rationale=(
                    "Retention is outside target and needs a named owner with a clear "
                    "recovery plan."
                ),
            ...
            SuggestedAction(
                ...
                rationale=(
                    "The board meeting needs current KPIs, decision points and "
                    "mitigations attached in advance."
                ),
                priority=Priority.high,
                owner="CEO Office",
                due_date=due,
            ...
            SuggestedAction(
                ...
                rationale=(
                    "Late-stage opportunities need executive sponsorship to increase "
                    "close probability."
                ),
