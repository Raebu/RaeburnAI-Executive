    ...
    async def important_emails(self) -> list[ExecutiveSignal]:
        return [
            ExecutiveSignal(
                ...
                summary=(
                    "A high-value prospect asked for pricing, timeline and security "
                    "evidence before Friday."
                ),
                priority=Priority.high,
                confidence=0.92,
                source_refs=[
                    SourceRef(provider="mock-gmail", title="Implementation plan request")
                ],
            ),
            ExecutiveSignal(
    ...
    async def calendar_signals(self) -> list[ExecutiveSignal]:
        return [
            ExecutiveSignal(
                ...
                summary=(
                    "Tomorrow's board meeting has no linked KPI pack or risk appendix "
                    "attached."
                ),
                priority=Priority.high,
                confidence=0.88,
    ...
    async def sales_signals(self) -> list[ExecutiveSignal]:
        return [
            ExecutiveSignal(
                ...
                summary=(
                    "Top-line pipeline is strong, but two late-stage deals have no "
    ...
    async def competitor_signals(
        ...
        return [
            ExecutiveSignal(
                ...
                summary=(
                    "Positioning overlaps with daily briefings and decision support. "
                    "Review differentiation and pricing."
                ),
                priority=Priority.medium,
                confidence=0.72,
                source_refs=[
                    SourceRef(provider="mock-market", title=f"{name} update")
                ],
            )
            for name in names[:3]
        ]
    ...
    async def news_signals(self) -> list[ExecutiveSignal]:
        return [
            ExecutiveSignal(
                ...
                summary=(
                    "Enterprise buyers are asking for stronger auditability, access "
                    "controls and AI risk reporting."
                ),
                priority=Priority.medium,
                confidence=0.8,
                source_refs=[
                    SourceRef(provider="mock-news", title="AI governance market signal")
                ],
            )
        ]
    ...
    async def risk_signals(self) -> list[ExecutiveSignal]:
        return [
            ExecutiveSignal(
                ...
                summary=(
                    "Churn is 1.3 percentage points above target and trending upward. "
                    "Investigate retention drivers."
                ),
                priority=Priority.high,
                confidence=0.9,
            ...
            ExecutiveSignal(
                ...
                summary=(
                    "Missing KPI and risk pack could reduce decision quality in "
                    "tomorrow's meeting."
                ),
                priority=Priority.medium,
                confidence=0.82,
