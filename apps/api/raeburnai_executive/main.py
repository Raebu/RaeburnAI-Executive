...
async def generate_daily_briefing(
    ...
    logger.info(
        "briefing_generated actor=%s organisation=%s", actor, request.organisation
    )
...
async def execute_action(
    ...
    audit_id = str(uuid4())
    logger.info(
        "action_execution_requested actor=%s audit_id=%s target=%s",
        actor,
        audit_id,
    ...
    return ActionExecutionResponse(
        ...
        message=(
            "Action recorded with human approval. Production connectors should execute "
            "this asynchronously."
        ),
        audit_id=audit_id,
    )
